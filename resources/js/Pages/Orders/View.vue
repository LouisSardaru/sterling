  ...
  <div>
    ...
    <div class="mt-2 flex items-center justify-between">
      ...
      <div class="flex flex-row items-center pl-2">
        ...
        <button v-tooltip.top="'Delete'" v-if="can.delete && !order.deleted_at" @click="deleteModalState = true" class="text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
          <svg class="w-6 h-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
            <path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
          </svg>
        </button>
        <button v-tooltip.top="'Restore'" v-else-if="can.restore && order.deleted_at" @click="restoreModalState = true" class="text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
          <svg class="w-6 h-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="my-4 bg-white shadow overflow-hidden rounded sm:rounded-lg">
      <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
        ...
        <p class="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
          Order details, type, status and total amount to be paid.
        </p>
      </div>
      <div class="px-4 py-5 sm:px-6">
        <dl class="grid grid-cols-1 col-gap-4 row-gap-8 sm:grid-cols-2">
          ...
          <div class="sm:col-span-1">
            ...
            <dd v-if="order.restaurant_table" class="mt-1 text-sm leading-5 text-gray-900">
          ...
          <div class="sm:col-span-1">
            ...
            <dd v-if="order.restaurant_table" class="mt-1 text-sm leading-5 text-gray-900 capitalize">
    ...
    <div class="my-4 bg-white shadow overflow-hidden rounded sm:rounded-lg">
      ...
      <div class="px-4 py-5 sm:px-6">
        <dl class="grid grid-cols-2 col-gap-4 row-gap-8">
          <template v-for="(item, index) in order.order_recipes">
            ...
            <div class="col-span-1" :key="`amount${index}`">
              <dt class="text-sm leading-5 font-medium text-gray-500">
                Amount
              </dt>
              <dd class="mt-1 text-sm leading-5 text-gray-900">
          ...
          <div v-if="!order.order_recipes.length" class="col-span-2 flex items-center justify-center py-2">
    ...
    <Modal :state="restoreModalState" @close="restoreModalState = false" @confirm="restore()" type="success">
      ...
      <p>
        Are you sure you want to restore <span class="font-medium">{{ order.name }}</span>?
      </p>
