  ...
  <div>
    <div class="flex items-center justify-between">
      ...
      <div v-if="can.create" class="flex">
        <span class="shadow-sm rounded-md">
          <inertia-link :href="route('users.create')" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-700 active:bg-indigo-700 transition duration-150 ease-in-out">
            Create
          </inertia-link>
        </span>
      </div>
    ...
    <div class="flex items-center justify-between mt-4">
      <div class="flex-1 min-w-0 max-w-lg">
        <TextInput v-model="form.search" label="Search users" :hidden-label="true" placeholder="Search users">
      ...
      <div class="flex pl-4">
        <Filters @reset="reset()">
          ...
          <div class="mt-2">
            <label for="role-filter" class="block text-sm font-medium leading-5 text-gray-700">
              Role
            </label>
            <div class="mt-1">
              <div class="max-w-lg rounded-md shadow-sm sm:max-w-xs">
                <select id="role-filter" ref="role-filter" v-model="form.role" class="capitalize block form-select w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                  <option value="all">
                    All
                  </option> 
          ...
          <table class="min-w-full">
            ...
            <tbody class="bg-white">
              <tr v-for="user in users.data" :key="user.id">
                ...
                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">
                  <span v-if="user.status" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                    Active
                  </span>
                ...
                <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                  <div class="flex flex-row justify-end items-center">
                    <inertia-link v-if="can.view" v-tooltip.top="'View'" :href="route('users.view', user.id)" class="mr-2 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
                    ...
                    <inertia-link v-if="can.edit" v-tooltip.top="'Edit'" :href="route('users.edit', user.id)" class="mr-2 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
                      <svg class="w-6 h-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                      </svg>
                    </inertia-link>
                    <button v-tooltip.top="'Delete'" v-if="can.delete && !user.deleted_at" @click="openDeleteModal(user.id, `${user.first_name} ${user.last_name}`)" class="text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
                      <svg class="w-6 h-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                      </svg>
                    </button>
                    <button v-tooltip.top="'Restore'" v-else-if="can.restore && user.delete.at" @click="openRestoreModal(user.id, `${user.first_name} ${user.last_name}`)" class="text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out">
                      <svg class="w-6 h-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <Modal :state="deleteModalState" @close="deleteModalState = false" @confirm="destroy(selectedUserId)" type="danger">
      <h1 slot="header"> 
        Delete account
      </h1>
      
      <p>
        Are you sure you want to delete <span class="font-medium">{{ selectedUserName }}'s</span> account?
    ...
    <Modal :state="restoreModalState" @close="restoreModalState = false" @confirm="restore(selectedUserId)" type="success">
      <h1 slot="header"> 
        Restore account
      </h1>
      
      <p>
        Are you sure you want to restore <span class="font-medium">{{ selectedUserName }}'s</span> account?
      </p>
...
export default {
  ...
  computed: {
    previousLink() {
      return this.users.links[0]
    },
    nextLink() {
      return this.users.links[this.users.links.length - 1]
    },
    lowerLimit() {
      if (((this.users.currentPage - 1) * this.form.pagination) + 1 > this.users.total) {
        return this.users.total
      }

      return ((this.users.currentPage - 1) * this.form.pagination) + 1
    },
    upperLimit() {
      if (this.users.currentPage * this.form.pagination > this.users.total) {
        return this.users.total
      }

      return this.users.currentPage * this.form.pagination
    },
  },
  watch: {
    form: {
  ...
  methods: {
    ...
    restore(id) {
      ...
      this.$inertia.put(this.route('users.restore', id))
    }
  },
}
</script>
